...
const Navbar = () => {
  ...
  const scrollTo = (link: string, e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    ...
    <nav className="w-full fixed top-0 left-0 flex flex-col justify-end px-4 z-50">
      ...
      <div className="flex justify-between z-50">
        <div className="flex items-center gap-32">
          {links.map((link: { link: string; text: string }, i: number) => (
            <Link
              key={i}
              href={link.link}
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <Plus
              className={`w-6 stroke-midBlue2 hover:stroke-lightBlue duration-200 align-middle ${isOpened && "-rotate-45"
                }`}
            ></Plus>
          </button>
      ...
      {/* Mobile Nav */}
      <div
        className={`px-4 py-32 block md:hidden absolute top-0 left-0 w-full h-[100lvh] backdrop-blur-md bg-[radial-gradient(169.40%_89.55%_at_94.76%_6.29%,rgba(0,0,0,0.40)_0%,rgba(255,255,255,0.00)_100%)]  z-40 duration-200 mobileMenu opacity-0 ${!isOpened && "pointer-events-none"
          }`}
      >
        <div className="w-full h-full flex flex-col justify-between">
          ...
          <button
            onClick={(e) => scrollTo("#footer", e)}
            className="block text-left darkBlueText hover:text-lightBlue duration-200 mobileNavLi opacity-0"
          >
            Contact
          </button>
        </div>
